...
object StreamingJoins {
  ...
  def joinStreamWithStatic() = {
    ...
    val streamedBandsGuitaristsDF = streamedBandsDF
      .join(guitarPlayers, guitarPlayers.col("band") === streamedBandsDF.col("id"), "inner")
  ...
  def joinStreamWithStream() = {
    ...
    val streamedGuitaristsBandsDF = spark.readStream
      .format("socket")
      .option("host", "localhost")
      .option("port", 12346)
      .load()
    ...
    val streamedJoin = streamedBandsDF.join(streamedGuitaristsBandsDF, streamedGuitaristsBandsDF.col("band") === streamedBandsDF.col("id"))
    ...
    streamedJoin.writeStream
      .format("console")
      .outputMode("append") // only append supported for stream vs strema join
      .start()
      .awaitTermination()
  }
